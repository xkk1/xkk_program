...
def change_information():
    ...
    # description = 简介 = information['description'].replace("\r\n", os.linesep)
    description = 简介 = information['description'].replace("\r\n", "\n").replace("\r", "\n")
...
封面链接：{封面链接}""")
            ...
            def restore_default():
                ...
                uid_entry.insert(tk.INSERT, default_data["uid"])
                title_length_entry.delete(0, tk.END)
                title_length_entry.insert(tk.INSERT, str(default_data["title_length"]))
                update_time_entry.delete(0, tk.END)
                update_time_entry.insert(tk.INSERT, str(default_data["update_time"]))
                form_text.delete("1.0", tk.END)
                form_text.insert(tk.INSERT, default_data["form"])

            restore_default_button = tk.Button(
                change_frame,
                text='恢复默认',
                font=('宋体', 15),
                command=restore_default,
                )
            restore_default_button.grid(row=5, column=0)
            ...
            def OK():
                ...
                try:
                    ...
                    data["program"] = default_data["program"]
                    data["author"] = default_data["author"]
                    save_data(data, json_data_file)  # 保存数据
